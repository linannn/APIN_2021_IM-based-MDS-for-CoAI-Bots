...
import logging
import sys
from top_grc_process import rptree_setup_top_grc
'''
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
def start_program(grc, support, mode, test_id):
    ...
    for ci in core_intent:
        ...
        elif (grc == 1) and (mode == 6):
            round, hit_content, recall = rptree_setup_top_grc(ci, mode, support, 1, test_id, 1)
        else:
...
def e4_e5_write(support, grc, mode, a, b):
    ...
    with open("./experiment_result/k" + str(support) + "/" + str(path) + "_" + str(a) + "_" + str(b) + ".txt", 'w', encoding='utf-8') as f1:
        f1.write('{:^10}{:^10}{:^15}{:^15}{:^15}{:^15}{:^15}'.format("NO.", "Round", "Ori_Content", "Lack_Content",
                                                               "Extra_Content", "Final_Cover", "Recall"))
        f1.write("\n")
        for test_id in range(a, b):
            print(test_id)
            round, hit_content, recall = start_program(grc, support, mode, test_id)
            ori_content = record[test_id - 1]
            # f1.write(str_e1)
            f1.write('{:^10}{:^10}{:^15}{:^15}{:^15}{:^15}{:^15}'.format(str(test_id), str(round), str(ori_content),
                                                                   str(ori_content - hit_content),
                                                                   str(0),
                                                                   str(hit_content / ori_content), str(recall)))
            f1.write("\n")
...
for support in range(2, 3):
    for grc in range(1, 2):
            ...
            e4_e5_write(support, grc, 6, 5001, end_id)
