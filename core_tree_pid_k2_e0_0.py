...
def start_program(grc, support, mode, test_id):
    if grc == 0:
        ...
        file_name = "./out_dia_process/k" + str(support) + "/e" + str(mode) + "/grc_" + str(test_id) + ".log"
    ...
    # print("您好，很高兴为您服务\n")
    logging.debug("To User:您好，很高兴为您服务")
    ...
    for ci in core_intent:
        ...
        # 在无法匹配的条件下直接执行free_qa
            ...
            # content, mode, support, round, test_id, hit_content
            model_tree, generate_tree, round, hit_content = rptree_setup_based_grc(ci, mode, support, 1, test_id, 0)
            rp_tree_set.append(model_tree)
            logging.info(test_id)
    ...
    if grc == 0:
        ...
        file_name_copy = "./out_dia_process/k" + str(support) + "/e" + str(mode) + "/grc_" + str(test_id) + "_final.log"
    copy_file(file_name, file_name_copy)
    print(round, hit_content)
