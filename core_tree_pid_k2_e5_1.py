...
def start_program(grc, support, mode, test_id):
    ...
    for ci in core_intent:
        ...
        # 在无法匹配的条件下直接执行free_qa
            ...
            # content, mode, support, round, test_id, hit_content
            model_tree, generate_tree, round, hit_content, recall = rptree_setup_based_grc(ci, mode, support, 1, test_id, 1)
            rp_tree_set.append(model_tree)
            logging.info(test_id)
...
# support k = 2; 3; 4; 5; 6
# support = 2
# use grc mode or not ; mode 4-grc-tb; 5-grc-bt
# grc = 0
for support in range(2, 3):
    for grc in range(1, 2):
            ...
            e4_e5_write(support, grc, 5, begin_id, 3001)
